...
class RaysGenerator:
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        """
        # Move pixel indices to the same device as self.images
        ...
        # Move images to GPU if not already there
        if self.images.device != self.device:
            self.images = self.images.to(self.device)
        
        color = self.images[img_idx][(pixels_y, pixels_x)]    # batch_size, 3
        
        if self.using_mask:
            # Make sure masks are on the same device
            if self.masks.device != self.device:
        ...
        # Remove the .cpu() calls since everything is already on GPU
