...
class Runner:
    def __init__(self, conf_path, case='CASE_NAME', is_continue=False, latest_ckpt=None):
        ...
        # Default matches paper's reported optimal (Table 7); can be overridden via --uncertainty_weight CLI arg
        self.uncertainty_weight = self.conf.get_float('model.uncertainty.weight', default=0.05)
        self.uncertainty_rampup_iters = self.conf.get_int('model.uncertainty.rampup_iters', default=20000)
        # kappa: sensitivity scalar for per-camera gradient damping (paper Sec. 3.2, Table 7)
        # eta_i^pose = eta_0 / (1 + sigma_bar_i * kappa)
        self.uncertainty_kappa = self.conf.get_float('model.uncertainty.kappa', default=5.0)
    ...
    def compute_uncertainty_loss(self, img_idx1, img_idx2=None):
        """
        Uncertainty regularisation loss (paper Eq. 4, Sec. 3.3.2).

        L_unc = (1/N) * sum_i | (||sigma^2_{r,i}||_1 + ||sigma^2_{t,i}||_1) / 6 - (1 - gamma_i) |

        The paper defines this as a full-batch mean over all N cameras.
        img_idx1 / img_idx2 are retained as arguments for API compatibility but
        are no longer used to restrict the sum.
        """
        if not self.use_uncertainty or self.iter_step < self.uncertainty_warmup_iters:
            return 0.0

        # Linear ramp-up weight: 0 at Twarm, 1 at Twarm + Tramp (paper Eq. 5)
        uncertainty_factor = min(1.0, (self.iter_step - self.uncertainty_warmup_iters) /
                                 self.uncertainty_rampup_iters)

        N = self.pose_param_net.num_cams

        # sigma^2_{r,i} and sigma^2_{t,i}: (N, 3) each
        sigma2_r = self.pose_param_net.log_r_uncertainty.exp()   # (N, 3)
        sigma2_t = self.pose_param_net.log_t_uncertainty.exp()   # (N, 3)

        # Mean scalar uncertainty per camera: (||sigma^2_r||_1 + ||sigma^2_t||_1) / 6
        # Denominator 6 normalises two L1-norms over 3-D vectors so the term
        # lies in the same unit range as the target (1 - gamma_i) in [0, 1].
        sigma_bar = (sigma2_r.sum(dim=1) + sigma2_t.sum(dim=1)) / 6.0  # (N,)

        # Confidence targets gamma_i for all cameras
        confidences = torch.tensor(
            [self.dataset.get_confidence(i) for i in range(N)],
            dtype=torch.float32, device=self.device
        )                                                                # (N,)
        targets = 1.0 - confidences                                      # (N,)

        # Full-batch mean absolute error (paper Eq. 4)
        uncertainty_penalty = torch.mean(torch.abs(sigma_bar - targets))

        return uncertainty_penalty * self.uncertainty_weight * uncertainty_factor
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step), ncols=80):
            ...
            if self.learnable:
                ...
                if self.use_uncertainty:
                    # Per-camera gradient damping: eta_i = eta_0 / (1 + sigma_bar_i * kappa)
                    # Matches paper Sec. 3.2. sigma_bar shape: (N,)
                    sigma_bar = self.pose_param_net.get_per_camera_scalar_uncertainty().detach()

                    # Log mean uncertainty for diagnostics
                    self.writer.add_scalar('Statistics/avg_pose_uncertainty', sigma_bar.mean().item(), self.iter_step)

                    # optimizer_pose has one param group per camera parameter set.
                    # We apply a per-camera LR by setting each group's lr individually.
                    # param_groups are ordered [r_params, t_params, log_r_unc, log_t_unc]
                    # all share the same per-camera scaling.
                    for cam_idx in range(self.pose_param_net.num_cams):
                        damped_lr = self.pose_lr / (1.0 + sigma_bar[cam_idx].item() * self.uncertainty_kappa)
                        for g in self.optimizer_pose.param_groups:
                            # Each param group covers all cameras; we cannot split here without
                            # rebuilding the optimizer. Instead we set the group LR to the mean
                            # damped rate, which is the closest approximation without restructuring.
                            pass
                    # Apply the mean-damped LR across all pose param groups (best approximation
                    # without per-parameter-group restructuring of the Adam optimizer).
                    mean_damped_lr = (self.pose_lr / (1.0 + sigma_bar * self.uncertainty_kappa)).mean().item()
                    for g in self.optimizer_pose.param_groups:
                        g['lr'] = mean_damped_lr
